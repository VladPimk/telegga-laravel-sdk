...
Реализованы HTTP-клиент, локальная модель подключения, создание подключения, явный повтор неудачной отправки, отправка текстовых сообщений, получение статуса сообщения и история сообщений пользователя. Остальные маршруты API и обработка вебхуков добавляются поэтапно.
